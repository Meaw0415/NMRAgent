...
Uses ChemBERTa tokenizer from HuggingFace.
"""
...
class NMR2SmilesDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            tokenizer: HuggingFace tokenizer (e.g., ChemBERTa)
        ...
        with env.begin() as txn:
            ...
            self.total_samples = len(all_keys)
        env.close()

        # Create split indices
        np.random.seed(seed)
        shuffled_indices = np.random.permutation(self.total_samples)

        val_num = 10000

        if split == 'val':
            #selected_indices = shuffled_indices[:val_num]
            selected_indices = shuffled_indices
        elif split == 'train':
            selected_indices = shuffled_indices[val_num:]

        # Map shuffled indices to actual LMDB keys
        self.keys = [all_keys[i] for i in selected_indices]

        print(f"[{split}] Samples: {len(self.keys)}")
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Get the actual LMDB key for this index
        key = self.keys[idx]

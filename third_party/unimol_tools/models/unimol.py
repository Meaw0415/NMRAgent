...
class UniMolModel(BaseUnicoreModel):
    def __init__(self, output_dim=2, material_structures_type='small_molecule', **params):
        super().__init__()
        
        if material_structures_type == 'small_molecule':
            self.args = molecule_architecture()
        elif material_structures_type == 'protein':
            self.args = protein_architecture()
        elif material_structures_type == 'inorganic_crystal':
            self.args = inorganic_crystal_architecture()
        elif material_structures_type == 'organic_crystal':
            self.args = organic_crystal_architecture()
        elif material_structures_type == 'mof':
            self.args = mof_architecture()
        else:
            raise ValueError('Current not support data type: {}'.format(material_structures_type))
        self.output_dim = output_dim
        self.material_structures_type = material_structures_type
        self.remove_hs = params.get('remove_hs', False)
        self.usecls = params.get('usecls', False)
        ...
        if material_structures_type == 'small_molecule':
                ...
                name = material_structures_type + '_' + name
                self.dictionary = Dictionary.load(os.path.join(WEIGHT_DIR, MODEL_CONFIG['dict'][name]))
                self.pretrain_path = MODEL_CONFIG['weight'][name][self.initial_weight]

                # if self.initial_weight == 'pretrain_self_supervised':
                #     self.pretrain_path = os.path.join(WEIGHT_DIR, MODEL_CONFIG['weight'][name])
                # elif self.initial_weight == 'none':
                #     self.pretrain_path = None
                # else :
                #     initial_weight_name = self.initial_weight + '.pth'
                #     self.pretrain_path = os.path.join(WEIGHT_DIR, initial_weight_name)
        else:
            self.pretrain_path = os.path.join(WEIGHT_DIR, MODEL_CONFIG['weight'][material_structures_type])
            self.dictionary = Dictionary.load(os.path.join(WEIGHT_DIR, MODEL_CONFIG['dict'][material_structures_type]))
        ...
        if material_structures_type == 'mof':
            self.min_max_key = {
            ...
            self.classifier = MolLevelPredictionHead(self.args.encoder_embed_dim+self.args.hidden_dim*5, 
                                    self.args.hidden_dim*2, 
                                    self.output_dim, 
                                    self.args.pooler_activation_fn,
                                    self.args.pooler_dropout)
        else:
            if self.task == 'atom_classification' or self.task == 'atom_multiclass' or self.task == 'atom_regression':
                if self.usecls:
                    self.prediction_head = AtomLevelPredictionHead_useCls(
                    ...
                    self.prediction_head = AtomLevelPredictionHead_onlyAtom(
                        num_tokens = self.max_tokens,
                        input_dim=self.args.encoder_embed_dim,
                        inner_dim=self.args.encoder_embed_dim,
                        activation_fn=self.args.pooler_activation_fn,
                ...
                self.prediction_head = MolLevelPredictionHead(
                    input_dim=self.args.encoder_embed_dim,
                    inner_dim=self.args.encoder_embed_dim,
    ...
    def load_pretrained_weights(self, path):
        if path is not None:
            if self.material_structures_type == 'mof':
    ...
    def forward(
        ...
        **kwargs
        ...
        # src_tokens = F.pad(src_tokens, (0, padding_length), value=0)
        # print(f"unimol.py {src_tokens.shape} src_tokens ")
        # padding_mask: padding到每个batch的最大原子数 + 1开头token + 1结尾token，[batch_size, batch_maxatom+2]→[batch_size, max_tokens]
        padding_mask = src_tokens.eq(self.padding_idx)
        # print(f"unimol.py padding_mask : {padding_mask.shape}")
        # mask中没有任何True值，则padding_mask 设置为None
        ...
        # print(f"unimol.py graph_attn_bias {graph_attn_bias.shape}")
        # 每个batch生成一次encoder_rep: torch.Size([batch_size, batch_maxatom+2, 512])→[batch_size, max_tokens, 512]
        (
            encoder_rep,
            _,
            _,
        ...
        # CLS token repr， cls是bos开头,[batch_size, 512]
        cls_repr = encoder_rep[:, 0, :]  
        # all token repr, 复制encoder_rep,
        all_repr = encoder_rep[:, :, :]  
        # print(f"unimol.py all_repr1 {all_repr.shape}")

        if return_repr:
            # filtered_tensors = []
            # for tokens in src_tokens:
            #     filtered_tensor = tokens[(tokens != 0) & (tokens != 1) & (tokens != 2)] # filter out 开头BOS(0), 结尾EOS(1), 填充PAD(2)
            ...
            #     atomic_repr = encoder_rep[i, 1:lengths[i]+1, :] # 第一维的第0个是cls_repr，后面是n个atom_repr
            #     cls_atomic_reprs.append(atomic_repr)


            # repr_dict = {'cls_repr': cls_repr, 'atomic_reprs': cls_atomic_reprs}  
            repr_dict = {'cls_repr': cls_repr, 'atomic_reprs': all_repr}        
              
            return repr_dict      

        # paddind：[batch_size, batch_maxatom+2, 512]→[batch_size, max_tokens, 512] 浪费显存，没有必要
        # padding_length = self.max_tokens - src_tokens.size(1)
        # padding = (0, 0, 0, padding_length)
        # all_repr = F.pad(all_repr, padding, "constant", 0)
        # print(f"unimol.py all_repr2 {all_repr.shape}")
        ...
        if self.material_structures_type == 'mof':
            ...
            if self.task == 'atom_classification' or self.task == 'atom_multiclass' or self.task == 'atom_regression':
                logits = self.prediction_head(all_repr)
            else:
                logits = self.prediction_head(cls_repr)
    ...
    def batch_collate_fn(self, samples):
        ...
        try:
            ...
            label = None
        try:
            mask = torch.tensor([s[2] for s in samples])
        except:
            mask = None
        return batch, label, mask

class MolLevelPredictionHead(nn.Module):
    """Head for mol-level Prediction tasks."""
    ...
    def __init__(
        ...
        self.out_proj = nn.Linear(inner_dim, num_tasks)
...
class AtomLevelPredictionHead_FlattenDNN(nn.Module):
    """Head for AtomLevel Prediction tasks."""
    ...
    def __init__(
        ...
        super().__init__()
        self.dense = nn.Linear(num_tokens*input_dim, inner_dim)
        self.activation_fn = get_activation_fn(activation_fn)
        self.dropout = nn.Dropout(p=pooler_dropout)
        self.out_proj = nn.Linear(inner_dim, num_tokens)
...
class AtomLevelPredictionHead_useCls(nn.Module):
    """Head for AtomLevel Prediction tasks."""
    ...
    def forward(self, features, **kwargs):
        ...
        maxatom = x.shape[1]
        len_withcls = 1024
        # 创建新的张量，形状为 [8, x, 1024]，初始化为零
        x_withcls = torch.zeros(batchsize, maxatom, len_withcls, dtype=x.dtype, device=x.device)
        # 复制原始张量到新张量的前 512 列
        x_withcls[:, :, :512] = x
        # 用 [i,0,:] 填充后 512 列
        x_withcls[:, :, 512:] = x[:, 0, :].unsqueeze(1).expand(-1, maxatom, -1)
        x = x_withcls
        # print(x.shape)
        # print(x[0,0,510:515])
        # x = self.dropout(x)
...
# onlyAtom
class AtomLevelPredictionHead_onlyAtom(nn.Module):
    """Head for AtomLevel Prediction tasks."""
...
def inorganic_crystal_architecture():
    args = argparse.ArgumentParser()
...
def organic_crystal_architecture():
    args = argparse.ArgumentParser()
    args.encoder_layers = getattr(args, "encoder_layers", 15)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)

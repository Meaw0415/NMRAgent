...
def cal_nan_metric(y_true, y_pred, mask = None, nan_value=None, metric_func=None):
    # current_dir = os.path.dirname(__file__)
    # parent_dir = os.path.join(current_dir, os.path.pardir)
    # config_path = os.path.join(parent_dir, 'config/default.yaml')
    # # config_path = os.path.join(os.path.dirname(__file__), 'config/default.yaml')
    # # print(f"metrics.py config_path{config_path}")
    # yamlhandler = YamlHandler(config_path)
    # config = yamlhandler.read_yaml()
    

    # print(f"mask_origin{mask[-1,:30]}")
    ...
    if mask is None:
        ...
        # 选择需要计算损失的 mask 值
        # selected_atoms = config.selected_atoms  # H-1 C-6 F-9
        # mask_bool = np.zeros_like(mask, dtype=bool)
        # # 根据 mask 选择需要计算损失的位置
        ...
        #         bool_mask = mask == selected_atom
        #         mask_bool = mask_bool | bool_mask
                
        mask = mask & (~np.isnan(y_true))
        if nan_value is not None:
            mask = mask & (y_true != nan_value)
    # print(f"y_true{y_true.shape} y_pred {y_pred.shape} mask{mask.shape}")
    # print(f"y_true{y_true[-1,:30]} y_pred {y_pred[-1,:30]} mask {mask[-1,:30]}")
    # sz = y_true.shape[1]
    # result = []
    # for i in range(sz):
    #     _mask = mask[:, i]
    ...
    # 将 y_true 和 y_pred 按照 mask 进行筛选
    y_true_selected = y_true[mask]
    y_pred_selected = y_pred[mask]
    # print(f"metric.py y_true_selected{y_true_selected.shape}  y_pred_selected{y_pred_selected.shape}")
    # print(f"metric.py y_true_selected{y_true_selected[:20]}  y_pred_selected{y_pred_selected[:20]}")
    try:
        result = metric_func(y_true_selected, y_pred_selected)    
    except Exception as e:
        print("An exception occurred while calculating the metric.")
        print("y_true_selected:", y_true_selected)
        print("y_pred_selected:", y_pred_selected)
        # 可以选择打印异常信息
...
class Metrics(object):
    ...
    def cal_classification_metric(self, label, predict, mask = None, nan_value=-1.0, threshold=None):
        r"""
            :param label:int
            :param predict:float
        """
        ...
        for metric_type, metric_value in self.metric_dict.items():
            ...
            def nan_metric(label, predict, mask = None): return cal_nan_metric(
                label, predict, mask=mask, nan_value=nan_value, metric_func=metric)
            if value_type == 'float':
                res_dict[metric_type] = nan_metric(
                    label.astype(int), predict.astype(np.float32), mask=mask)
            elif value_type == 'int':
                thre = 0.5 if threshold is None else threshold
                res_dict[metric_type] = nan_metric(
                    label.astype(int), (predict > thre).astype(int), mask=mask)

        # TO DO : add more metrics by grid search threshold

        return res_dict

    def cal_reg_metric(self, label, predict, mask = None, nan_value=-1.0):
        r"""
            :param label:int
            :param predict:float
        """
        # print(f"cal_reg_metric mask_origin{mask[-1,:30]}")
        res_dict = {}
        for metric_type, metric_value in self.metric_dict.items():
            metric, _, _ = metric_value
            def nan_metric(label, predict, mask = None): return cal_nan_metric(
                label, predict, mask=mask, nan_value=nan_value, metric_func=metric)
            res_dict[metric_type] = nan_metric(label, predict, mask=mask)

        return res_dict

    def cal_multiclass_metric(self, label, predict, mask = None, nan_value=-1.0, label_cnt=-1):
        r"""
            :param label:int
            :param predict:float
        """
    ...
    def cal_metric(self, label, predict, mask = None, nan_value=-1.0, threshold=0.5, label_cnt=None):
        # print(f"metric.py label {label.shape} {label[mask][-20:]}")
        # print(f"metric.py predict {predict.shape} {predict[mask][-20:]}")

        if self.task in ['regression', 'multilabel_regression', 'atom_regression']:
            # print(f"cal_metric mask_origin{mask[-1,:30]}")

            return self.cal_reg_metric(label, predict, mask = mask, nan_value=nan_value)
        elif self.task in ['classification', 'multilabel_classification']:
            return self.cal_classification_metric(label, predict, mask = mask, nan_value=nan_value)
        elif self.task in ['multiclass']:
            return self.cal_multiclass_metric(label, predict, mask = mask, nan_value=nan_value, label_cnt=label_cnt)
        else:
    ...
    def _judge_early_stop_decrease(self, wait, score, min_score, model, dump_dir, fold, patience, epoch):
        ...
        if score <= min_score:
            ...
            # print(f"metrics.py epoch {epoch} {os.path.join(dump_dir, f'model_{fold}.pth')}")

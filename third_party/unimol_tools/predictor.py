...
class UniMolRepr(object):
    def __init__(self, material_structures_type='small_molecule', initial_weight='pretrain_self_supervised', remove_hs=False, use_gpu=True):
        self.device = torch.device("cuda:0" if torch.cuda.is_available() and use_gpu else "cpu")
        self.model = UniMolModel(output_dim=1, material_structures_type=material_structures_type, task = 'repr', initial_weight=initial_weight, remove_hs=remove_hs).to(self.device)
        self.model.eval()
        self.params = {'material_structures_type':material_structures_type, 'remove_hs': remove_hs}

    def get_repr(self, data, datatype = 'smiles', batch_size = 32):
        # print(datatype)
        if datatype == 'smiles':
            ...
            dataset = MolDataset(datahub.data['unimol_input'], None, datahub.data['atom_mask'])

            self.trainer = Trainer(task='repr', batch_size=batch_size)
            ...
            datahub = DataHub(data = point3d_dict, 
                            datatype = datatype,
                            task = 'repr', 
                            is_train = False, 
                            **self.params,
...
class MOFPredictor(object):
    def __init__(self, use_gpu=True):
        self.device = torch.device("cuda:0" if torch.cuda.is_available() and use_gpu else "cpu")
        self.model = UniMolModel(output_dim=1, material_structures_type='mof').to(self.device).half()
        self.model.eval()

...
class ConformerGen(object):
    ...
    def _init_features(self, **params):
        ...
        if self.material_structures_type == 'small_molecule':
            name = "no_h" if self.remove_hs else "all_h" 
            name = self.material_structures_type + '_' + name
            self.dict_name = MODEL_CONFIG['dict'][name]
        else:
            self.dict_name = MODEL_CONFIG['dict'][self.material_structures_type]
        self.dictionary = Dictionary.load(os.path.join(WEIGHT_DIR, self.dict_name))
        self.dictionary.add_symbol("[MASK]", is_special=True)

    def single_process(self, data, datatype='smiles'):
        if self.method == 'rdkit_random':
            atoms, coordinates = inner_smi2coords(data, datatype, seed=self.seed, mode=self.mode, remove_hs=self.remove_hs)
    ...
    def transform(self, data, datatype='smiles'):
        pool = Pool()
        logger.info('Start generating conformers...')
        # 创建一个偏函数，将 datatype 与 self.single_process 绑定
        partial_func = partial(self.single_process, datatype=datatype)
        # 使用偏函数进行并行处理
        inputs = [item for item in tqdm(pool.imap(partial_func, data))]
        pool.close()
...
def inner_smi2coords(data, datatype='smiles', seed=42, mode='fast', remove_hs=False):
    # print(f"inner_smi2coords:{remove_hs}")
    remove_hs = False
    if datatype == 'smiles':
    ...
    # print(f"mol中的原子数：{len(atoms)}")
    assert len(atoms)>0, 'No atoms in molecule: {}'.format(data)
        ...
        # 没有自带构象则按照原来的方式
            ...
            if res == 0:
                try:
                    ...
                    print("can not use MMFF optimize")
    ...
    assert len(atoms) == len(coordinates), "coordinates shape is not align with {}".format(data)
    if remove_hs:
        # print("生成3D构象时去除了H")
        idx = [i for i, atom in enumerate(atoms) if atom != 'H']
        atoms_no_h = [atom for atom in atoms if atom != 'H']
        coordinates_no_h = coordinates[idx]
        assert len(atoms_no_h) == len(coordinates_no_h), "coordinates shape is not align with {}".format(data)
        return atoms_no_h, coordinates_no_h
    else:
        # print("生成3D构象时没有去除H")
        return atoms, coordinates

def inner_coords(atoms, coordinates, remove_hs=False):
...
def coords2unimol(atoms, coordinates, dictionary, max_atoms=510, **params):
    # array(['C', 'H'], dtype='<U1')
    ...
    ### tokens为原子的字典索引并添加 bos="[CLS]"开头，eos="[SEP]"结尾，还没开始padding
    src_tokens = np.array([dictionary.bos()] + [dictionary.index(atom) for atom in atoms] + [dictionary.eos()])
    # print(f"conformer.py src_tokens{src_tokens.shape}")
...
def coords2unimol_mof(atoms, coordinates, dictionary, max_atoms=510):
    atoms = np.array(atoms)
    coordinates = np.array(coordinates).astype(np.float32)
    ### cropping atoms and coordinates
    if len(atoms)>max_atoms:
